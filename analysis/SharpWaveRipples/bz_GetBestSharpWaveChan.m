...
mSW = zeros(1, n_channels);
meSW = zeros(1, n_channels);
mmSWRatio = zeros(1, n_channels);
...
if sum(isnan(filt_test)) > 100
    ...
    disp('Lower filter order had to be used to find best SW channel')
end
% check if it still failed
filt_test = FiltFiltM(b,a,single(lfp.data(:,1)));
if sum(isnan(filt_test)) >100 
    error('Filter failed when searching for best SW channel')
end
...
x_ripple = coords(best_ripple_channel+1, 1);
y_ripple = coords(best_ripple_channel+1, 2);
